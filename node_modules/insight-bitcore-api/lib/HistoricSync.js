...
HistoricSync.prototype.getBlockFromRPC = function(cb) {
  ...
  self.rpc.getBlock(self.currentRpcHash, function(err, ret) {
    ...
    if (ret) {
      ...
      if(self.info().syncPercentage > 98) {
	self.broadcast(blockInfo);	
      }
...
HistoricSync.prototype.broadcast = function(blockInfo) {
	sockets.broadcastBlock(blockInfo.hash);	
	sockets.broadcastTx(blockInfo.tx.toString());
};
...
HistoricSync.prototype.checkNetworkSettings = function(next) {
  ...
  self.rpc.getBlockHash(0, function(err, res){
    if (!err && ( res && res.result !== self.genesis)) {
      //var genesisHashReversed = new Buffer(res.result, 'hex');
      //res.copy(genesisHashReversed);
      //buffertools.reverse(genesisHashReversed);
      err = new Error(BAD_GEN_ERROR + config.network + " Loaded genesis: " + self.genesis + " - From RPC: "+ res.result);
    }
